...
build:  ## Build all executables. Example: make build
	go build -a -mod=vendor -race -ldflags $(LDFLAGS) -o _output/subscription-injection-webhook ./cmd
.PHONY: build

build-image: ## Build the images and push them to the remote registry. Example: make build-images
	rm -rf _output
	$(CONTAINER_RUNTIME) build -f Dockerfile -t $(REGISTRY)/$(REPOSITORY)/origin-subscription-injection-webhook:$(TAG) .
	$(CONTAINER_RUNTIME) push $(REGISTRY)/$(REPOSITORY)/origin-subscription-injection-webhook:$(TAG)
.PHONY: build-image
